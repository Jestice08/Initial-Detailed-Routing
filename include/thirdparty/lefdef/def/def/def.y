      ...
      orient
        {
          if (defData->callbacks->RowCbk) {
            defData->rowName = $3;
            defData->Rowmap.setup($3, $4, $5, $6, $7);
          }
        }
      row_do_option
      row_options ';'
        {
          if (defData->callbacks->RowCbk) 
            CALLBACK(defData->callbacks->RowCbk, defrRowCbkType, &defData->Rowmap);
        }
      ...
      | K_DO NUMBER K_BY NUMBER row_step_option
        {
          ...
          if (defData->callbacks->RowCbk)
            defData->Rowmap.setDo(ROUND($2), ROUND($4), defData->xStep, defData->yStep);
        }
...
row_prop : T_STRING NUMBER
        {
          if (defData->callbacks->RowCbk) {
             ...
             sprintf(str, "%g", $2);
             defData->Rowmap.addNumProperty($1, $2, str, propTp);
          }
        }
      | T_STRING QSTRING
        {
          if (defData->callbacks->RowCbk) {
             ...
             propTp =  defData->session->RowProp.propType($1);
             CHKPROPTYPE(propTp, $1, "ROW");
             defData->Rowmap.addProperty($1, $2, propTp);
          }
        }
      | T_STRING T_STRING
        {
